...
Name:  TODO: Greg Matthews and Ke Yang
...
(define (not-simp expr)
  (cond
    [(equal? expr #t) #f]
    [(equal? expr #f) #t]
    [(and {list? expr} (is-not? expr)) (last expr)]
    [(list 'not expr)]))
...
(define (and-simp expr1 expr2)
  (cond
    [(and (equal? expr1 #t) (equal?  expr2 #t)) #t]
...
(define (or-simp expr1 expr2)
  (cond
    [(and (equal? expr1 #f) (equal? expr2 #f)) #f]
...
(define (bool-simp expr)
  (cond
   ...
   [(is-or? expr) (or-simp (bool-simp (op1 expr)) (bool-simp (op2 expr)))]
   [(is-and? expr) (and-simp (bool-simp (op1 expr)) (bool-simp (op2 expr)))]
))
...
To prove the above property, we need to prove '(not expr) = (not-simp expr). We give the proof by case analysis as below: 
expr = constant:
        expr = #t: 
        Leftside:  '(not expr) = #f
        Rightside: (not-simp expr)= #f


        expr = #f: 
        Leftside: '(not expr)= #t
        Rightside: (not-simp expr) = #t


expr is a variable:
        Leftside: '(not expr)
        Rightside: (not-simp expr) = '(not expr) by definition


expr is an AND expression:
        Leftside: '(not expr)
        Rightside: (not-simp expr) = '(not expr) by definition


expr is a OR expression:
        Leftside: '(not expr)
        Rightside: (not-simp expr) = '(not expr) by definition


expr is a NOT expression:
        Leftside: '(not expr) = '(not (not last-expr)) = last-expr
        Rightside: (not-simp expr) = (last expr) = last-expr by definition


2)  (bool-eval '(and expr1 expr2) env) =
        (bool-eval (and-simp expr1 expr2) env)
Similar to proof of (1), we need to prove '(and expr1 expr2) = (and-simp expr1 expr2) 
in order to prove 2). We also give the proof by case analysis as below.


expr1 and expr2 both are constants:
        expr1 = #t, expr2 = #t: 
        Leftside:  '(and expr1 expr2) = #t
        Rightside: (and-simp expr1 expr2) = #t


one of expr is constant:
        expr1 = #t, expr2 = ALL(can be anything except #t proved above): 
        Leftside: '(and expr1 expr2) = '(and #t expr2) = expr2
        Rightside: (and-simp expr1 expr2) = expr2


        expr1 = #f, expr2 = ALL(can be anything): 
        Leftside: '(and expr1 expr2) = '(and #f expr2) = #f
        Rightside: (and-simp expr1 expr2) = #f


        expr1 = ALL(can be anything except constants except #t proved above), expr2 = #t: 
        Leftside: '(and expr1 expr2) = '(and expr1 #t) = expr1
        Rightside: (and-simp expr1 expr2) = expr1


        expr1 = ALL(can be anything except constants), expr2 = #f: 
        Leftside: '(and expr1 expr2) = '(and expr1 #f) = #f
        Rightside: (and-simp expr1 expr2) = #f


expr1 is a variable, expr2 is a variable:
        Leftside: '(and expr1 expr2)
        Rightside: (and-simp expr1 expr2) = '(and expr1 expr2) by definition


expr1 is other cases (AND, OR, NOT sub-expr), expr2 is other cases (AND, OR, NOT sub-expr):
        Leftside: '(and expr1 expr2)
        Rightside: (and-simp expr1 expr2) = '(and expr1 expr2) by definition
...
Similar to proof of (1) and (2), we need to prove '(or expr1 expr2) = (or-simp expr1 expr2) 
in order to prove 3). We also give the proof by case analysis as below.


expr1 and expr2 both are constants:
        expr1 = #f, expr2 = #f: 
        Leftside:  '(or expr1 expr2) = #f
        Rightside: (or-simp expr1 expr2) = #f


one of expr is constant:
        expr1 = #t, expr2 = ALL(can be anything): 
        Leftside: '(or expr1 expr2) = '(or #t expr2) = #t
        Rightside: (or-simp expr1 expr2) = #t


        expr1 = #f, expr2 = ALL(can be anything except #f proved above): 
        Leftside: '(or expr1 expr2) = '(or #f expr2) = expr2
        Rightside: (or-simp expr1 expr2) = expr2


        expr1 = ALL(can be anything), expr2 = #t: 
        Leftside: '(or expr1 expr2) = '(or expr1 #t) = #t
        Rightside: (or-simp expr1 expr2) = #t


        expr1 = ALL(can be anything except #f proved above), expr2 = #f: 
        Leftside: '(or expr1 expr2) = '(or expr1 #f) = expr1
        Rightside: (or-simp expr1 expr2) = expr1


expr1 is a variable, expr2 is a variable:
        Leftside: '(or expr1 expr2)
        Rightside: (or-simp expr1 expr2) = '(or expr1 expr2) by definition
...
In order to prove (4), we need to prove expr = (bool-simp expr). We give the following case analysis and induction hypnosis. 


Assume that expr_i = (bool-simp expr_i) for expr_i that has tree size less than expr in the representation of binary tree of boolean function.


Base cases:
expr = constant:
        Leftside: expr 
        Rightside: (bool-simp expr) = expr
expr is a variable:
        Leftside: expr 
        Rightside: (bool-simp expr) = expr


General cases(induction):
expr is a NOT:
        Leftside: expr is (not expr1) 
        Rightside: (bool-simp expr) = (not-simp (bool-simp (op1 expr)))
                                    = (not-simp expr1) by IH
                                    = (not expr1) by above proved (1)        
expr is a AND:
        Leftside: expr is (and expr1 expr2) 
        Rightside: (bool-simp expr) = (and-simp (bool-simp (op1 expr)) (bool-simp (op2 expr)))
                                    = (and-simp expr1 expr2) by IH and expr1 is (op1 expr) and expr2 is (op2 expr).
                                    = (and expr1 expr2) by above proved (2)  
expr is a OR:
        Leftside: expr is (or expr1 expr2)
        Rightside: (bool-simp expr) = (or-simp (bool-simp (op1 expr)) (bool-simp (op2 expr)))
                                    = (or-simp expr1 expr2) by IH and expr1 is (op1 expr) and expr2 is (op2 expr).
                                    = (or expr1 expr2) by above proved (3)
...
(define (no-double-negatives? expr)
  (cond
    ...
    [(is-variable? (op1 expr)) #t]

    [(is-and? expr) (and (no-double-negatives?(op1 expr)) (no-double-negatives?(op2 expr)))]
    [(is-or? expr) (and (no-double-negatives?(op1 expr)) (no-double-negatives?(op2 expr)))]
    [(and (is-not? expr) (is-not? (op1 expr))) #f]
    [else #t]
))
...
(define (is-simplified? expr)
  (cond
    [(and (or (is-constant? expr) (no-constants? expr)) (no-double-negatives? expr)) #t]
    [else #f]
))
...
To prove part4, we need following lemmas.
...
Prove by cases analysis:
 E1 is constant:
 (not-simp E1) is also a constant which is simplified.
 E1 is variable:
 (not-simp E1) returned ('not E1) which is simplified by assumption.
 E1 is a NOT expression:
 (not-simp E1) returned (last E1) which is simplified by assumption.
 E1 is an AND/OR expression:
 (not-simp E1) returned ('not E1) which is simplified by assumption.


Lemma 2: (is-simplified? E1 ) and (is-simplified E2) -> (is-simplified? (and-simp E1 E2)


Prove by cases analysis:
 E1 and E2 are constants:
 (and-simp E1 E2) is also a constant which is simplified.
 E1 is constant (E2 is constant similar to this):
 (and-simp E1 E2) is #f if E1 is #f. 
 (and-simp E1 E2) is E2 if E1 is #t. E2 is simplified by assumption.
 E1 and E2 are variables or expressions:
 (and-simp E1 E2) returned ('and E1 E2) which is simplified because both E1 and E2 are simplified and neither constants.


Lemma 3: (is-simplified? E1 ) and (is-simplified E2) -> (is-simplified? (or-simp E1 E2)


Prove by cases analysis:
 E1 and E2 are constants:
 (or-simp E1 E2) is also a constant which is simplified.
 E1 is constant (E2 is constant similar to this):
 (or-simp E1 E2) is #t if E1 is #t. 
 (or-simp E1 E2) is E2 if E1 is #f. E2 is simplified by assumption.
 E1 and E2 are variables or expressions:
 (or-simp E1 E2) returned ('or E1 E2) which is simplified because both E1 and E2 are simplified and neither constants.


With above lemmas, we can prove part 4 (is-simplified? (bool-simp expr)) as below.
Base cases:
        expr = constant:
                 (is-simplified? (bool-simp expr))
           =(is-simplified? constant)  by def of bool-simp and is-constant
           = #t by def of is-simplified
        expr is a variable:
                 (is-simplified? (bool-simp expr))
           =(is-simplified? variable)  by def of bool-simp and is-variable
           = #t by def of is-simplified
General cases (induction):
        We assume (is-simplified E1) and (is-simplified E2) are both true by induction hypnosis. 
        expr is a NOT expression:
                (is-simplified? (bool-simp expr))
           =(is-simplified? (not-simp (bool-simp (op1 expr))))  by def of bool-simp
       =(is-simplified? (not-simp E1)) by def of bool-simp and E1 is (op1 expr).
       = #t by IH and lemma 1


    expr is an AND expression:
                (is-simplified? (bool-simp expr))
           =(is-simplified? (and-simp (bool-simp (op1 expr)) (bool-simp (op2 expr))))  by def of bool-simp
       =(is-simplified? (and-simp E1 E2)) by def of bool-simp and E1 is (op1 expr) and E2 is (op2 expr).
       = #t by IH and lemma 2


    expr is a OR expression:
                (is-simplified? (bool-simp expr))
           =(is-simplified? (or-simp (bool-simp (op1 expr)) (bool-simp (op2 expr))))  by def of bool-simp
       =(is-simplified? (or-simp E1 E2)) by def of bool-simp and E1 is (op1 expr) and E2 is (op2 expr).
       = #t by IH and lemma 3
